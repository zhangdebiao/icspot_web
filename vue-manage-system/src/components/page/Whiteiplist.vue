    ...
    <div class="table">
        ...
        <div class="container">
	 	 ...
	 	 <el-form-item style="float: right; margin-top:-10px;">
        		 <el-button type="primary" @click="PostWhiteiplist" >提交配置</el-button>
	         </el-form-item>
            <p style="font-size:12px;font-family:'PingFang SC';color:grey; margin-top: 7px;">举例：192.168.1.1,192.168.1.2。端口之间用英文逗号分隔。</p>
...
<script>
    ...
    export default {
	    ...
	    PostWhiteiplist() {
                // 开发环境使用 easy-mock 数据，正式环境使用 json 文件
                if (process.env.NODE_ENV === 'development') {
                    this.ipurl = process.env.API_HOST+'/whiteiplist/';
                    this.$axios.post(this.ipurl,{
                        ip : this.whiteips,
                    })
                    .then((res) => {
                        this.$message.success('提交成功！');
